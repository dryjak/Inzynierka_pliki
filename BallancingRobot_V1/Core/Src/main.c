...
float EncoderMaxValue = 200.0;
float EncoderMinValue = -200.0;
...
float AngleMaxValue = 30.0f;
float AngleMinValue = -30.0f;
...
void MoveMotors(void)
{
	...
	if(PidMotorAPidValue > 0)
	{
		DirA = 1;	//kierunek do Tyłu (odwrotnie połączone niż motorB)
		PwmInputA = PidMotorAPidValue;
	}
	else
	{
		DirA = 0;	//kierunek do Przodu
		PwmInputA = PidMotorAPidValue;
	}

	if(PidMotorBPidValue > 0)
	{
		DirB = 1;	//kierunek do przodu
		PwmInputB = PidMotorBPidValue;
	}
	else
	{
		DirB = 0;	//kierunek do tyłu
		PwmInputB = PidMotorBPidValue;
	}

	//RPM to PWM
	float PwmMotorA = MapValues(EncoderMaxValue, PwmInputA);
	float PwmMotorB = MapValues(EncoderMaxValue, PwmInputB);
